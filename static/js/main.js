...
function shareResults() {
    ...
    const shareText = `🌍 Tourism Plan for ${placeInput}\n\n${outputContent}\n\nGenerated by Tourism Planning Assistant`;
...
function printResults() {
    ...
    printWindow.document.write('<h1>🌍 Trip Plan: ' + placeInput + '</h1>');
...
function saveTrip() {
    ...
    if (!placeName || !outputContent) {
        showNotification('⚠️ No trip to save!');
        return;
    }
    ...
    if (exists) {
        showNotification('ℹ️ Trip already saved!');
        return;
    }
...
function deleteTrip(tripId) {
    ...
    localStorage.setItem('savedTrips', JSON.stringify(savedTrips));
    displaySavedTrips();
    showNotification('🗑️ Trip deleted!');
}
...
function displaySearchHistory() {
    ...
    searchHistory.slice(0, 5).forEach(place => {
        html += `<span class="chip" onclick="quickSearch('${place}')">🕒 ${place}</span>`;
    });
    html += '</div>';
...
function displayPopularDestinations() {
    ...
    popularDestinations.slice(0, 5).forEach(place => {
        html += `<span class="chip chip-popular" onclick="quickSearch('${place}')">✨ ${place}</span>`;
    });
    html += '</div>';
    ...
    // Default: Show everything including AI features
        ...
        if (data.travel_tips) {
            ...
            output += `<div class="travel-tips-box">
                <div class="travel-tips-title">💡 Travel Tips</div>
                <div>${tipsHtml}</div>
            </div>`;
        }
        ...
        if (data.itinerary) {
            ...
            output += `<div class="ai-summary-box" style="border-left-color: #48bb78; background: linear-gradient(135deg, #f0fff4, #e8f5e9); margin-top: 25px;">
                <div class="ai-summary-title" style="color: #2f855a;">📅 Suggested Itinerary</div>
    ...
    attractions.forEach((attraction) => {
        ...
        html += `
            <div class="attraction-info">
                ...
                ${attraction.address ? `<div class="place-address">📍 ${attraction.address}</div>` : ''}
            </div>
        </div>`;
    });
